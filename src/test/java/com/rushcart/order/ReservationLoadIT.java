 ...
 * and records oversell count + latency percentiles for the current locking strategy
 * (optimistic, via {@link Inventory}'s {@code @Version}).
 */
class ReservationLoadIT extends AbstractIntegrationTest {

    private static final String SKU = "SKU-LOAD-TEST";
    ...
    @Test
    void optimisticLockingUnderBurstLoad() throws InterruptedException {
        ...
        Callable<Void> task = () -> {
            ...
            try {
                orderService.reserveOptimistic(UUID.randomUUID(), SKU, 1);
        ...
        System.out.printf(
                "%n[§13 Optimistic Locking] requests=%d success=%d insufficientStock=%d conflict=%d"
                        + " p95=%dms p99=%dms%n",
